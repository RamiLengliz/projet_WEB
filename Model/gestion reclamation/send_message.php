...
if (
    ...
    if($chat->getIdUser() == 1){
        header('Location: ../../view/admin/index.php?page=chat&id_reclamation='.$_POST['id_reclamation'].'&id_user='.$_POST['id_user_dest']);
    }else
    header('Location: ../../view/Student/template/course-message.php'); // Redirect with success indicator
    exit;
} else {
    header('Location: ../../view/Student/template/course-message.php?result=4'); // Redirect with error indicator for empty fields
    exit;
}
?>
