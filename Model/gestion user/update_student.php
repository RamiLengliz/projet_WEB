...
if (isset($_POST['email'], $_POST['name'], $_POST['lastname'], $_POST['age'], $_POST['class'])) {
    // Validate non-empty fields
    if (!empty($_POST['email']) && !empty($_POST['name']) && !empty($_POST['lastname']) && !empty($_POST['age']) && !empty($_POST['class'])) {
       ...
       $result = $userC->updateUser($id,$user->getEmail(),$user->getRole(),$user->getName(),$user->getLastname(),password_hash($password, PASSWORD_DEFAULT), $user->getStatus(),$user->getAge());
        if ($result){
            $userC->update_etu($id,$classID);
            header('Location: ../../view/admin/index.php?page=CS&result=1');
        }else{
            header('Location: ../../view/admin/index.php?page=CS&result=2');
        }
        ...
        //sendEmail($email, $name, $emailSubject, $emailBodyHtml, $emailBod
    } else {
        header('Location: ../../view/admin/index.php?page=CS&result=3'); // Missing information
    }
} else {
    header('Location: ../../view/admin/index.php?page=CS&result=3'); // Missing POST keys
}
?>
