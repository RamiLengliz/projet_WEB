...
class ResultatCon {
    ...
    public function getNote($id_examen,$id_user) {
        $sql = "SELECT * FROM $this->tab_name WHERE id_examen = :id_examen and id_user = :id_user";
        $db = config::getConnexion();
    ...
    public function listResultatUser($id)
    {
        $sql = "SELECT * FROM $this->tab_name WHERE $id = id_user";
        $db = config::getConnexion();
        try {
            $query = $db->query($sql);
