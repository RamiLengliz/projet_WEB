...
<body>
    ...
    $messages = $chatC->listMessagesId_user($_SESSION['user_id']); // Fetch all messages
    $id_student = $_GET['id_user'];
    $user = $userC->getUser($id_student);
    ?>
    ...
    <div class="chat-container p-3">
        <div class="chat-messages p-2" id="chat-messages">
            ...
            <div class="d-flex flex-column gap-2">
                <?php foreach ($messages as $message): 
                    if($message['id_user'] == $id_student || $message['id_user_dest'] == $id_student){
                    ?>
                    
                <div class="p-2 bg-light border rounded <?= $message['id_user'] == $_SESSION['user_id'] ? 'ms-auto' : '' ?>">
                    <strong><?= htmlspecialchars($message['id_user'] == $_SESSION['user_id'] ? 'You' : $user['name'].' '.$user['lastname']) ?>:</strong>
        ...
        <div class="chat-form mt-3">
            <form id="chat-form" action="../../model/gestion reclamation/send_message.php" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="id_user_dest" value="<?=$id_student;?>"> <!-- Assume destination user ID -->
